...
- 新增：关闭软件时支持“每次询问 / 后台播放 / 直接退出”，后台播放会隐藏窗口但继续播放，并提供 Windows 托盘图标用于恢复或退出。
- 新增：关闭确认弹窗支持“不再提示，记住我的选择”。
- 修复：启动自动播放恢复本地歌曲时，补回 `coverPath`、`hasCover` 和内嵌封面读取，避免重启后歌曲封面丢失。
- 新增：视觉控制台内存管理在窗口隐藏或最小化后真正触发播放器工作集压缩，并优化内存区字体与按钮布局。
- 移除：视觉控制台完整移除“播放输出”功能及其无用脚本。
- 优化：壁纸模式、全屏热键、桌面图标可见性和视觉控制台入口的交互稳定性。

## v2.1.0

- 新增：视觉控制台「系统」页支持自定义本地音乐文件存放地址，默认 `E:\MyMusic`，保存后用于本地扫描、整理和主页读取。
- 新增：播放栏均衡器改为悬停展开，管理自定义 EQ 可直接跳转到系统页对应分组。
- 优化：自定义 EQ 编辑窗口改为高级深色音频工作站风格。
- 优化：视觉控制台各设置分组默认收起，点击分组标题后展开。

## v2.0.2

- 修复：红心状态三处双向同步。左侧本地歌曲列表点红心后，播放栏红心与歌单队列红心立即同步点亮/熄灭；反向（播放栏点红心）时左侧列表红心也同步更新。
- 修复：歌单/队列面板中已红心的歌曲，鼠标悬停时不再变白（新增 `#playlist-panel .qi-act button.liked:hover` 规则，hover 保持红色系）。
- 修复：本地歌曲列表行尾「添加到歌单」加号按钮改为对称 SVG 图标，鼠标悬停放大时十字加号不再偏移，始终严格居中。
- 修复：缓存目录（节拍分析、在线元数据）不再生成在程序文件夹外，统一放在 Mineradio 文件夹内的 `MineradioCache` 目录；仍可用环境变量 `MINERADIO_BEAT_CACHE_DIR` / `MINERADIO_META_CACHE_DIR` 覆盖。

## v2.0.1

- 修复：本地歌曲列表行尾「添加到歌单」圆形加号按钮不再居中（`text-indent:1px` + `padding:0 0 2px 0` 导致偏右偏上），去掉偏移后加号在圆钮内严格居中。
- 新增：退出时保存使用设置。播放模式（顺序循环 / 随机播放 / 单曲循环）持久化到本地，切换即保存、退出（beforeunload/pagehide）再保存一次，下次启动自动恢复上次的播放模式（含 UI 图标与「歌单/队列」面板状态）。
