...
function updateWallpaperEngineEntryUi(message) {
  ...
  if (value) {
    ...
    else if (active) value.textContent = (wallpaperEngineSelection.title || '已选择') + ' · 已应用为背景';
...
function applyWallpaperEngineBackground(item, quiet) {
  ...
  function beginWallpaperEngineMediaLoad() {
    ...
    if (kind === 'engine' && wallpaperEngineNativeHostUnavailable()) {
      return;
    }
    clearWallpaperEngineLayerMedia(0);
    if (kind === 'engine') {
  ...
  if (!quiet) showToast(kind === 'engine' ? '正在用 Wallpaper Engine 原生引擎载入 Scene…' : (kind === 'media' ? '壁纸已应用为背景' : '已应用壁纸预览为背景'));
  return true;
}
...
function activateWallpaperEngineItem(id) {
  ...
  // 互动 Scene 壁纸用 WE 实时引擎渲染；图片/视频壁纸直接铺进应用背景。
  var selectKind = item.enginePlayable ? 'engine' : (item.playable ? 'media' : 'preview');
  var reuseSame = wallpaperEngineSelection && wallpaperEngineSelection.active
    && wallpaperEngineSelection.id === String(id || '')
    && !wallpaperEngineRuntimeError
    && (selectKind !== 'engine' || !!wallpaperEngineNativeSessionId);
  if (reuseSame) {
  ...
  wallpaperEngineSelection = normalizeWallpaperEngineSelection({
    ...
    title: item.title,
    kind: selectKind,
    mediaType: selectKind === 'engine' ? 'video' : (selectKind === 'media' ? item.mediaType : 'image'),
...
async function openWallpaperEngineLibrary() {
  // 弹窗 HTML 在脚本标签之后才解析，初始化时网格还不存在；
  // 每次打开时重绑一次（内部有去重标记，不会重复绑定）。
...
function initializeWallpaperEngineLibrary() {
  bindWallpaperEngineLibraryEvents();
  updateWallpaperEngineEntryUi();
  // 升级版不自动恢复上次壁纸：避免“开机自动启动 + 用户点击切换”
  // 双启动互相取代、窗口超时的竞态。用户点击壁纸后再启用。
  wallpaperEngineSelection = normalizeWallpaperEngineSelection({
    active: false,
    id: String(wallpaperEngineSelection && wallpaperEngineSelection.id || ''),
    title: String(wallpaperEngineSelection && wallpaperEngineSelection.title || ''),
    kind: 'preview',
    mediaType: 'image',
    hasPreview: false,
    updatedAt: wallpaperEngineSelection && wallpaperEngineSelection.updatedAt || 0
  });
  return;
}
