...
function ensureMemoryFxDefaults() {
  ...
  fx.memorySystemIntervalMin = clampRange(Math.round(fx.memorySystemIntervalMin == null ? (fxDefaults && fxDefaults.memorySystemIntervalMin) || 30 : Number(fx.memorySystemIntervalMin)), 5, 180);
  fx.memorySystemThresholdPercent = clampRange(Math.round(fx.memorySystemThresholdPercent == null ? (fxDefaults && fxDefaults.memorySystemThresholdPercent) || 78 : Number(fx.memorySystemThresholdPercent)), 50, 98);
  fx.memorySystemMask = normalizeMemorySystemMask(fx.memorySystemMask == null ? (fxDefaults && fxDefaults.memorySystemMask) || MEMORY_REDUCT_MASK_DEFAULT : fx.memorySystemMask);
}
...
function bindSystemMemoryControls() {
  ...
  [
    ...
    input.addEventListener('input', function () {
      fx[item[1]] = clampRange(Math.round(Number(input.value) || (fxDefaults && fxDefaults[item[1]]) || 30), item[2], item[3]);
