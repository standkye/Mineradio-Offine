...
class WallpaperEngineRuntime {
  ...
  async embedActiveWindow(expectedSessionId = '', host = {}) {
    ...
    const operation = (async () => {
      ...
      const hostSettleTolerance = 6;
      const embedDeadline = this.now() + 26000;
      let previousHostRect = null;
      let stableSince = null;
      let attempt = 0;
      while (embedding.aligned !== true && this.now() <= embedDeadline) {
        if (generation !== this.generation || this.disposed || this.active !== session || session.stopping === true) {
          throw runtimeError('WALLPAPER_ENGINE_START_SUPERSEDED');
        }
        const hostRect = [
          Number(embedding && embedding.hostLeft) || 0,
          Number(embedding && embedding.hostTop) || 0,
          Number(embedding && embedding.hostRight) || 0,
          Number(embedding && embedding.hostBottom) || 0,
        ];
        const hostMoved = previousHostRect && hostRect.some((value, index) => Math.abs(value - previousHostRect[index]) > hostSettleTolerance);
        previousHostRect = hostRect;
        if (hostMoved) {
          // The host window is still being dragged/resized. Do not relaunch the
          // native source toward a moving target; wait for it to settle first.
          stableSince = null;
          await this.nativeSleep(220);
          embedding = await this._controlSessionWindow('embed', session, String(session.sourceId || ''), host);
          continue;
        }
        const hostStable = stableSince == null || (this.now() - stableSince) >= 480;
        if (!hostStable) {
          if (stableSince == null) stableSince = this.now();
          await this.nativeSleep(160);
          embedding = await this._controlSessionWindow('embed', session, String(session.sourceId || ''), host);
          continue;
        }
        ...
        const scaleX = Math.max(0.5, Math.min(4, (Number(session.width) || hostWidth) / hostWidth));
        const scaleY = Math.max(0.5, Math.min(4, (Number(session.height) || hostHeight) / hostHeight));
